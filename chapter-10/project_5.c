...
void analyze_hand(int hand[NUM_CARDS][2])
{
  ...
  for (int card = 0; card < NUM_CARDS; card++){
    if (hand[card][1] != base_suit){
      flush = false;
      break; // instantly break if second card suit does not match first, no need to check more cards
    }
  }
  ...
  */
  ...
  /* 
    ...
    sum of all cards in a low ace straight 
    should always be 18,
    (ex. sum of cards 2, 3, 4, 5, ace =  0 (2) + 1 (3) + 2 (4) + 3 (5) + 12 (ace) = 18
  */
  int low_ace_sum = 0;
  for (int card = 0; card < NUM_CARDS; card++){
    low_ace_sum += hand[card][0];
  }

  if(highest_card - lowest_card == 4 || low_ace_sum == 18){
    straight = true;
  }

  if (straight == true && highest_card == 12 && lowest_card == 8){
    royal = true;
  }
...
void print_result(void)
{
  ...
  else if (royal)        printf("Royal straight");
