...
int main(void)
{
  ...
  char date_str[7], time_str[7], msg_str[MSG_LEN+1];
  ...
  for (;;) {
    ...
    printf("Enter day, month, time and reminder: ");
    scanf(" %2d/ %2d", &day, &month);
    if (day == 0 || month == 0)
      break;
    else if ( day < 0 || day > 31){
      printf("Error: day must be in range 0-31\n");
      while(getchar() != '\n'); // clear buffer
      continue;
    }

    sprintf(date_str, "%2d/%-2d\t", day, month);
